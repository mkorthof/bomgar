...
if [ $( id -u ) -eq 0 ]; then
  id -u $bomusr >/dev/null 2>&1 || useradd -d $bomdir -c jumpclient $bomusr -M
  ...
  which update-rc.d >/dev/null 2>&1 && update-rc.d bomgar start 20 2 3 4 5 . stop 20 0 1 6 .
  which chkconfig >/dev/null 2>&1 && chkconfig --add bomgar
  [ -f $bomdir/init-script ] && $bomdir/init-script start && echo Bomgar Jump Client was started successfully
else
